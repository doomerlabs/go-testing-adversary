#!/usr/bin/env node
import { createRequire as __goTestingCreateRequire } from 'node:module'; const require = __goTestingCreateRequire(import.meta.url);
...
var domain = {
  ...
  rules: [
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} TestMain function${count === 1 ? "" : "s"} never execute the package tests.`,
      whyItMatters: "Without m.Run(), the package's tests silently never execute \u2014 CI exits 0 having tested nothing.",
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} fatal-style test call${count === 1 ? "" : "s"} run on a background goroutine.`,
      whyItMatters: "FailNow calls runtime.Goexit on the wrong goroutine \u2014 the test can pass despite the failure, or hang.",
    ...
    },
    {
      ...
      severity: "high",
      confidence: "high",
      summary: (count) => `${count} parallel test scope${count === 1 ? "" : "s"} also mutate the process environment.`,
      whyItMatters: "Go deliberately panics when the same test or subtest combines Parallel with Setenv because environment variables are process-wide.",
      impact: "The test binary panics before the assertions run, blocking the package test suite.",
      recommendation: "Keep the test serial, or inject configuration without mutating the process environment; changing the call order does not make this combination safe."
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} test wait${count === 1 ? "" : "s"} rely on a fixed sleep for synchronization.`,
      whyItMatters: "Sleeps race the scheduler: too short flakes; long enough slows the suite.",
      impact: "Flaky CI under load and permanently slow tests.",
      recommendation: "Synchronize with channels/WaitGroups, or poll with a deadline \u2014 never a bare sleep."
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} test listener${count === 1 ? "" : "s"} bind a fixed port.`,
      whyItMatters: "Fixed ports collide under parallel tests, repeated runs, and shared CI runners.",
    ...
    },
    {
      ...
      severity: "medium",
      confidence: "high",
      summary: (count) => `${count} environment mutation${count === 1 ? "" : "s"} bypass test-scoped restoration.`,
      whyItMatters: "Leaked env mutations cause order-dependent failures that only reproduce in full-suite runs.",
    ...
    },
    {
      ...
      severity: "low",
      confidence: "high",
      summary: (count) => `${count} test${count === 1 ? "" : "s"} are permanently skipped with no condition.`,
  ...
  analyze(file) {
    ...
    const unsafeSetenvLines = new Set(
      parallelSetenv.map((signal) => signal.data.setenvLine).filter((line) => typeof line === "number")
    );
    return {
      signals: [
...
function fatalInGoroutineSignals(file) {
  const signals = [];
  signals.push(
    ...lineSignals(
      file,
...
function parallelSetenvSignals(file) {
  const code = maskNonCode(file.current);
  const scopes = [];
  ...
  for (const scope of scopes) {
    ...
    const body2 = code.slice(bodyStart, scope.closeBrace);
    const parallelOffset = directMethodCall(body2, scope.variable, "Parallel");
    const setenvOffset = directMethodCall(body2, scope.variable, "Setenv");
    if (parallelOffset === void 0 || setenvOffset === void 0) continue;
...
function unconditionalSkipSignals(file) {
  const signals = [];
  const re = /\bfunc\s+(Test\w*)\s*\(\s*\w+\s+\*testing\.T\s*\)\s*\{/g;
  let match;
  while ((match = re.exec(file.current)) !== null) {
    const start2 = (match.index ?? 0) + match[0].length;
    const rest = file.current.slice(start2);
    const lines = rest.split("\n");
    for (let i2 = 0; i2 < Math.min(lines.length, 12); i2 += 1) {
...
function provenCallbackExecutors(root, source) {
  ...
  for (const fn of functions) {
    ...
    for (const parameter of parameters.namedChildren) {
      ...
      const text2 = sourceText(parameter, source).trim();
      const match = text2.match(/^([A-Za-z_]\w*)\s+func\s*\(/);
...
function stringValue(node, source) {
  const value = transparent(node);
  if (value === void 0) return void 0;
  const text2 = sourceText(value, source);
  if (value.type === "raw_string_literal") return text2.slice(1, -1);
...
function stringValue2(node, source) {
  ...
  if (node.type === "raw_string_literal") return text2.slice(1, -1);
  try {
    return JSON.parse(text2);
  } catch {
    return void 0;
...
import { dirname as dirname5, join as join5, relative as relative3, sep as sep2 } from "node:path";
...
async function siblingTestContext(repository, directories, knownPaths, limit) {
  ...
  for (const directory of [...directories].sort()) {
    ...
    const absoluteDirectory = await safeExistingPath(root, join5(root, ...directory.split("/")));
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      try {
        const absolute = await safeExistingPath(root, join5(absoluteDirectory, entry.name));
        if (absolute === void 0) continue;
        const metadata2 = await stat(absolute);
        if (!metadata2.isFile() || metadata2.size > MAX_FILE_BYTES) continue;
        const current = await readFile6(absolute, "utf8");
